 ...
 *
 * Carried over from the llamaindex harness on request. The two doc pages are
 * byte-identical apart from their slug, so nothing had to be ported — only
 * `AGENT_ID`, which is `my_agent` there (its Quickstart's agent) and
 * `agentic_chat` here.
 *
 * One thing that behaves differently on this integration: the `<select>` is fed
 * by `useThreads`, which returns an empty list unless CopilotKit Intelligence is
 * configured. Without a key you get "No threads yet" and both Open buttons stay
 * disabled — the readout and New chat still work, since those are pure client
 * state. See the route notes.
 */
